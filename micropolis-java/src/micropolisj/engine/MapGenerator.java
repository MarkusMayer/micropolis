...
public class MapGenerator {
	...
	static enum CreateIsland {
		NEVER, ALWAYS, SELDOM; // seldom == 10% of the time
	}
	...
	/**
	 * Level for tree creation. If positive, this is (roughly) the number of trees
	 * to randomly place. If negative, then the number of trees is randomly chosen.
	...
	private void makeNakedIsland() {
		...
		for (int y = 0; y < WORLD_Y; y++) {
			for (int x = 0; x < WORLD_X; x++) {
				map.setSpec(MapPosition.at(x, y), Tiles.get(RIVER));
			}
		}

		for (int y = 5; y < WORLD_Y - 5; y++) {
			for (int x = 5; x < WORLD_X - 5; x++) {
				map.setSpec(MapPosition.at(x, y), Tiles.get(DIRT));
			}
		}
	...
	private void doRivers() {
		...
		doBRiv();
//		System.out.println("after doBRiv: "+map);
		mapX = xStart;
		mapY = yStart;
...
//		System.out.println("after doBRiv: "+map);
	...
	static final char[][] BRMatrix = new char[][] { { 0, 0, 0, 3, 3, 3, 0, 0, 0 }, { 0, 0, 3, 2, 2, 2, 3, 0, 0 },
			{ 0, 3, 2, 2, 2, 2, 2, 3, 0 }, { 3, 2, 2, 2, 2, 2, 2, 2, 3 }, { 3, 2, 2, 2, 4, 2, 2, 2, 3 },
			{ 3, 2, 2, 2, 2, 2, 2, 2, 3 }, { 0, 3, 2, 2, 2, 2, 2, 3, 0 }, { 0, 0, 3, 2, 2, 2, 3, 0, 0 },
			{ 0, 0, 0, 3, 3, 3, 0, 0, 0 } };
	...
	static final char[][] SRMatrix = new char[][] { { 0, 0, 3, 3, 0, 0 }, { 0, 3, 2, 2, 3, 0 }, { 3, 2, 2, 2, 2, 3 },
	...
	private void putOnMap(char mapChar, int xoff, int yoff) {
		...
		int tmp = map.getTileNr(MapPosition.at(xloc, yloc));
		if (tmp != DIRT) {
			tmp &= LOMASK;
		...
		map.setSpec(MapPosition.at(xloc, yloc), Tiles.get(mapChar));
	}

	static final char[] REdTab = new char[] { RIVEDGE + 8, RIVEDGE + 8, RIVEDGE + 12, RIVEDGE + 10, RIVEDGE + 0, RIVER,
			RIVEDGE + 14, RIVEDGE + 12, RIVEDGE + 4, RIVEDGE + 6, RIVER, RIVEDGE + 8, RIVEDGE + 2, RIVEDGE + 4,
			RIVEDGE + 0, RIVER };

	private void smoothRiver() {
		for (int mapY = 0; mapY < getHeight(); mapY++) {
			for (int mapX = 0; mapX < getWidth(); mapX++) {
				if (map.getTileNr(MapPosition.at(mapX, mapY)) == REDGE) {
					int bitindex = 0;
					...
					for (int z = 0; z < 4; z++) {
						...
						int xtem = mapX + DX[z];
						int ytem = mapY + DY[z];
						if (engine.testBounds(xtem, ytem) && (map.getTileNr(MapPosition.at(xtem, ytem)) != DIRT)
								&& (((map.getTileNr(MapPosition.at(xtem, ytem)) < WOODS_LOW)
										|| ((map.getTileNr(MapPosition.at(xtem, ytem)) > WOODS_HIGH))))) {
							bitindex |= 1;
						}
					}
					...
					if ((temp != RIVER) && PRNG.nextInt(2) != 0)
						temp++;
					map.setSpec(MapPosition.at(mapX, mapY), Tiles.get(temp));
				}
			}
		}
	}
	...
	private void treeSplash(int xloc, int yloc) {
		...
		for (int z = 0; z < dis; z++) {
			...
			if (map.getTileNr(MapPosition.at(mapX,mapY))== DIRT) {
				map.setSpec(MapPosition.at(mapX,mapY),Tiles.get(WOODS));
			}
		}
	}
	...
	static final char[] TEdTab = new char[] { 0, 0, 0, 34, 0, 0, 36, 35, 0, 32, 0, 33, 30, 31, 29, 37 };

	private void smoothTrees() {
		for (int mapY = 0; mapY < getHeight(); mapY++) {
			for (int mapX = 0; mapX < getWidth(); mapX++) {
				if (isTree(map.getTileNr(MapPosition.at(mapX,mapY)))) {
					int bitindex = 0;
					for (int z = 0; z < 4; z++) {
						bitindex <<= 1;
						int xtem = mapX + DX[z];
						int ytem = mapY + DY[z];
						if (engine.testBounds(xtem, ytem) && isTree(map.getTileNr(MapPosition.at(xtem,ytem)))) {
							bitindex |= 1;
						}
					}
